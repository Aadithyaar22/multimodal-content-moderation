...
Cross-attention − late fusion = **−0.0027 macro-F1**, t=−0.553, **p=0.596** — indistinguishable from noise, and numerically in the *wrong* direction for the thesis. On its own, this reads as "cross-attention does nothing here." The control below shows that reading is incomplete.

### 6.4.1 Isolating the mechanism from capacity and granularity

A fair objection to the table above: cross-attention differs from late fusion in three ways at once, not one. It has roughly 18–25x late fusion's own parameter count (3,687,173 vs. 148,613 on Hateful Memes at `d_model=256`; 13,926,405 vs. 791,045 on Fakeddit at `d_model=512`), and it reads unpooled, token-level features that late fusion never sees at all. A performance gap attributed to "the fusion mechanism" without controlling for the other two is not a clean measurement.

**No cross-attn (control)** (`arch="no_cross_attention"`, `src/mcm/models/fusion.py::CrossAttentionBlock`) isolates the mechanism by construction rather than by post-hoc statistical adjustment: it is the identical `CrossAttentionBlock` module, with query/key/value routed *within* each modality (self-attention) instead of across modalities when `cross_modal=False`. This keeps parameter count exactly identical to cross-attention at a matching configuration — not approximately matched, verified byte-for-byte equal via `sum(p.numel() for p in model.parameters())` and asserted in `tests/test_fusion.py::TestNoCrossModalControl` — and keeps the same token-level input granularity. The only thing that changes is whether information can flow *between* modalities at all. Whatever the control does differently from late fusion is attributable to capacity and granularity alone; whatever cross-attention adds on top of the control is attributable to the mechanism specifically.

| Dataset | Capacity + granularity alone (control − late fusion) | Attention specifically (cross-attn − control) |
|---|---|---|
| Hateful Memes | +0.0058 macro-F1, t=1.048, p=0.319 — not significant | +0.0067 macro-F1, t=1.050, p=0.315 — not significant |
| Fakeddit | **−0.0157 macro-F1, t=−3.196, p=0.013 — significant, control is *worse*** | **+0.0130 macro-F1, t=2.965, p=0.018 — significant, cross-attention is *better*** |

Fakeddit is the more informative result. Added capacity and finer-grained input, *without* a mechanism to relate the two modalities, makes performance significantly worse than late fusion (p=0.013) — on a benchmark where much of the misinformation signal already lives in the text alone, a larger token-level model with no cross-modal coupling has more room to overfit and no corresponding benefit. Restoring cross-modal attention at the identical parameter count then recovers a statistically significant **+0.0130 macro-F1 (p=0.018)** — real, measurable work attributable to the mechanism, not to noise. It does not fully close the gap back to late fusion's leaner approach (§6.4's headline p=0.596 stands, unchanged), which is why the raw comparison reads as "no effect." That framing is accurate about the net outcome and incomplete about the mechanism: cross-attention is significantly counteracting a significant capacity penalty on Fakeddit, and mostly succeeding.

On Hateful Memes, neither half of the decomposition reaches significance individually (p=0.319, p=0.315) — consistent with splitting an already-borderline headline result (p=0.051) into two smaller pieces at unchanged seed count, which predicts reduced power on each half rather than a resolved answer. The point estimate splits the +0.0125 headline gain roughly evenly between capacity/granularity (46%) and the mechanism (54%). This narrows *where* the remaining uncertainty lives without resolving it; §9.5 and §10 are updated accordingly.
...
The full five-arm table, including the no-cross-attention capacity/granularity control, is in §6.4–§6.4.1. Restated as the single most important figure in this report: cross-attention fusion outperforms the late-fusion baseline on Hateful Memes by 1.25 macro-F1 points, a gap that is directionally consistent with the fusion thesis but falls just short of conventional significance (p=0.051, 7 seeds). On Fakeddit the two are statistically indistinguishable overall (p=0.596) and numerically reversed — but the parameter-matched control (§6.4.1) decomposes that null result rather than leaving it unexplained: added capacity without cross-modal attention is *significantly worse* than late fusion there (p=0.013), and cross-attention *significantly recovers* most of that loss (p=0.018). The mechanism is doing real, measurable work on both benchmarks; on Fakeddit it is fighting a capacity penalty to a near-draw rather than doing nothing. §9.5 discusses why a flow-specific, honest reading of this — rather than a single blended conclusion — is the more useful one.
...
**Researcher/ablation.** The full master comparison table is in §6.4–§6.4.1, including parameter counts fine-tuned per arm: 133,637 for each unimodal arm (identical across both benchmarks and both modalities, by design — see §6.1), 3,687,173 for cross-attention *and* its no-cross-attention control on Hateful Memes (`d_model=256`, verified identical), 13,926,405 for cross-attention *and* its control on Fakeddit (`d_model=512`, verified identical).
...
- **The headline ablation result is not statistically significant on either benchmark against late fusion directly** (Hateful Memes p=0.051, Fakeddit p=0.596, reversed in direction). The parameter-matched no-cross-attention control (§6.4.1) refines rather than resolves this: on Fakeddit, the null result is not "the mechanism does nothing" — added capacity and token-level input *without* cross-modal attention is significantly worse than late fusion (p=0.013), and cross-attention significantly recovers most of that loss (+0.0130, p=0.018). The mechanism does real, measurable, statistically significant work there; it is just fighting a capacity penalty to a near-draw rather than posting a net win. On Hateful Memes, the same decomposition splits the (still non-significant) headline gain roughly 46/54 between capacity/granularity and the mechanism, with neither half individually significant at 7 seeds — expected given that splitting an already-borderline result reduces power on each piece. This is consistent with the fusion thesis being about *which cases* fusion helps on, not a universal improvement, but current sample sizes (5-7 seeds) cannot fully separate either benchmark's decomposition from seed noise at conventional significance.
...
This project set out to test a specific, falsifiable claim: that keeping per-modality representations alive through cross-attention lets a model learn harm patterns that exist only in the *relationship* between an image and a caption, in a way a late-fusion baseline — which has already collapsed each modality to a score before combining — structurally cannot. The honest result is a **partial, benchmark-dependent confirmation, sharpened by a parameter-matched control that isolates the mechanism from capacity and input granularity (§6.4.1)**. On Hateful Memes, a benchmark purpose-built so that neither modality is offensive alone, cross-attention shows the predicted direction of improvement over late fusion but does not clear conventional statistical significance at the sample size trained here (p=0.051, 7 seeds); decomposing that gap splits it roughly evenly between added capacity/granularity and the mechanism itself, with neither half individually significant. On Fakeddit, the raw comparison against late fusion is indistinguishable (p=0.596) — but the control shows this is not "the mechanism does nothing." Added capacity and token-level input without cross-modal attention is *significantly worse* than late fusion there (p=0.013), and cross-attention *significantly recovers* most of that loss (p=0.018). The mechanism does real, measurable work on both benchmarks; on Fakeddit that work is legible only once capacity is controlled for, since it manifests as damage control rather than a net win. Reporting either the raw headline gap or the decomposition alone would have been the easiest way to oversimplify the project's central claim in either direction; reporting both as they actually measured is the more defensible contribution.

Around that central result, the system built to test it turned out to be a substantial deliverable in its own right: a decision-support API (never a black-box auto-moderator, enforced at the type level) with two independently working explainability methods, one of which (text occlusion) is measurably faithful by a controlled deletion test with a random-region baseline, not merely plausible; a working image-reuse detector with an empirically-derived threshold; a deployed, cost-bounded cloud service; and a test suite (148+ backend tests at time of writing, including the parameter-identity and information-isolation checks in `TestNoCrossModalControl`) built substantially around real bugs this project encountered and fixed during its own development — a CORS-wiping deploy-config bug caught twice, an operator-precedence bug caught before it shipped, a genuine 12-minute cold-start regression caught by actually redeploying and checking, and the `active_heads` blind spot (§9.4, Case C) caught on a real analyzed item rather than a synthetic test case.

**Future work**, in the order it would most change the headline conclusion: (1) more training seeds on the Hateful Memes cross-attention and no-cross-attention arms specifically, since both the headline gap (p=0.051) and its decomposition (p=0.319, p=0.315) are close enough to significance that a modest increase in seed count could resolve them either way; (2) a real human-agreement study now that the infrastructure correctly attributes decisions to distinct raters (`docs/human-agreement-study.md`); (3) a larger-n faithfulness eval for the image modality, where the signed direction is right but the current sample size cannot separate it from chance; (4) provisioning `MONGODB_URI` so moderator decisions and the resulting agreement statistics persist past a redeploy; and (5), if a public Xception/FaceForensics++ checkpoint becomes available, replacing the current ViT-based deepfake classifier to close the one remaining documented deviation from the original architecture brief.
...
# Capacity/granularity vs. mechanism decomposition (§6.4.1)
python scripts/ablation_table.py --datasets hateful_memes fakeddit --baseline late_fusion --proposal no_cross_attention
python scripts/ablation_table.py --datasets hateful_memes fakeddit --baseline no_cross_attention --proposal cross_attention
...
Raw results backing every table in this report are committed at `reports/results/*.json` — the ablation runs (`cross_attention__*.json`, `late_fusion__*.json`, `no_cross_attention__*.json`, etc., one file per architecture/dataset/seed) and `faithfulness.json` (per-dataset, per-item deletion-test results).
