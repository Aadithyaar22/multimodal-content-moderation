...
export function ModalityLadder({ scores, signal, task, threshold = 0.5 }: Props) {
  const value = (k: (typeof RUNGS)[number]["key"]) => scores[k]?.[task] ?? 0;
          ...
          return (
            <div key={rung.key}>
              <div className="mb-2 flex items-baseline justify-between">
                ...
                <span
                  className={`numeric font-display text-lg font-bold ${
                    isFusion ? "text-on-surface" : "text-on-surface-variant"
                  }`}
                >
                  {v.toFixed(2)}
                </span>
              ...
              <p className="mt-1.5 text-xs text-outline">
                {clears ? "clears threshold" : "below threshold"}
              </p>
            </div>
          );
        })}
